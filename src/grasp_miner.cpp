...
int main(int argc, char** argv)
{
  ...
  const double threshold = varmap["threshold"].as<double>();
  ...
  ConstHeuristic hybrid_heuristic(problem, hybrid_generator, selector);
  Solver solver(
      init_heuristic
    , local_search
    , data_miner
...
boost::program_options::variables_map ParseArgs(int argc, char* argv[])
{
  ...
  visible_opts.add_options()
    ...
    ("threshold,t", po::value<double>()->default_value(0.5), "proportion of solutions to mine");
